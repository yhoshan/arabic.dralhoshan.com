...
const outputPath = "/home/ubuntu/archive-diwans-api-expanded.json";
const auditPath = "/home/ubuntu/archive_diwan_api_expanded_audit.json";
...
const requestPlan = [
  {
    name: "العناوين التي تصرح بكلمة ديوان بلا حصر وسم اللغة",
    query: "mediatype:texts AND title:ديوان",
  },
  {
    name: "الموضوع أو الوصف الذي يصرح بالديوان بلا حصر وسم اللغة",
    query: "mediatype:texts AND (subject:ديوان OR description:ديوان)",
  },
  {
    name: "عناوين شعرية تقرنها البيانات الوصفية بديوان أو مجموعة شعرية",
    query: "mediatype:texts AND (title:قصائد OR title:أشعار OR title:اشعار OR title:شعر) AND (subject:ديوان OR description:ديوان OR subject:\"مجموعة شعرية\" OR description:\"مجموعة شعرية\")",
  },
  {
    name: "البيانات الوصفية التي تصرح بديوان شعري أو مجموعة شعرية",
    query: "mediatype:texts AND (subject:\"ديوان شعري\" OR description:\"ديوان شعري\" OR subject:\"مجموعة شعرية\" OR description:\"مجموعة شعرية\")",
  },
];

const rejectTitle = /(?:دراسه|رساله|اطروحه|بحث|شرح|فهرس|معجم|نقد|تحليل|قراءه|موازنه|اتجاهات|ببليوغرافيا|مكتبه|موسوعه|تاريخ|سجل|اداره|مجله|صحيفه|جريده|مقال|مقدمه|بحوث|ملحوظات|(?:ذيل|تكمله|تكملة|مستدرك|استدراك)\s+(?:على\s+)?ديوان|ديوان\s+(?:السنه|السنة|السنن|الحديث|الاحاديث|الأحاديث|الاثار|الآثار|الضعفاء|المتروكين|الزكاه|الزكاة|العرب)(?:\s|$)|و(?:ما\s+وصل\s+الينا\s+من\s+)?نثره|ورسائله|واخباره|ديوان\s+(?:المبتدا|المبتدأ|الخبر|العبر|الانشاء|الإنشاء|الرسائل|الخراج|المحاسبه|المحاسبة|المظالم|العدل|الخدمه|الخدمة|المال|الماليه|المالية|الشكاوي|الشكاوى|الوزاره|الوزارة|الدوله|الدولة|الوقف|الحساب|الاموال|الأموال|العقارات|الضرائب)|ديوان\s+(?:الشعر\s+العربي|الشعر\s+العربى|اللصوص|الفكاهه|الفكاهة|الادب|الأدب|النثر\s+العربي|المعاني))/u;
const contextualRejectTitle = /(?:^|\s)(?:دراسات|التكوينات|توظيف|الفائزون|جائزه)(?:\s|$).*?(?:^|\s)ديوان(?:\s|$)|(?:^|\s)(?:مع\s+)?تحقيق\s+ديوان(?:\s|$)|ديوان\s+(?:الاساطير|الاحكام|اسماء\s+(?:الضعفاء|المتروكين))(?:\s|$)|ديوان(?:\s|$).*?(?:رواياتي|روايات)(?:\s|$)/u;
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      const response = await fetch(url, {
        headers: { "user-agent": "Arabic-Language-Thesaurus/1.2 (metadata-only deep cataloging)" },
        signal: AbortSignal.timeout(90000),
      });
      if (!response.ok) throw new Error(`${response.status} ${response.statusText}`);
      const payload = await response.json();
      if (!payload.response || !Array.isArray(payload.response.docs)) {
        throw new Error("استجابة بحث متقدم غير صالحة من Internet Archive");
      }
      return {
        items: payload.response.docs,
        total: Number(payload.response.numFound ?? 0),
        start: Number(payload.response.start ?? 0),
      };
    } catch (error) {
      lastError = error;
...
for (const plan of requestPlan) {
  let page = 1;
  let returnedForPlan = 0;
  let reportedTotal = null;

  while (returnedForPlan < maxRecordsPerQuery) {
    try {
      const payload = await fetchBatch(plan, page);
      const current = payload.items;
      reportedTotal = payload.total;
      for (const doc of current) if (doc.identifier) docs.set(doc.identifier, doc);
      returnedForPlan += current.length;
      const hasNextPage = Boolean(current.length && returnedForPlan < Math.min(reportedTotal, maxRecordsPerQuery));
      requests.push({
        name: plan.name,
        query: plan.query,
        page,
        returned: current.length,
        cumulativeReturned: returnedForPlan,
        reportedTotal,
        hasNextPage,
      });
...
const materials = [...chosen.values()]
  ...
  .map((item) => ({
    ...
    matchEvidence: {
      ...
      explicitLanguageSource: asList(item.doc.language).some((value) => ["ara", "arabic"].includes(String(value).toLowerCase())),
    },
  }));
...
const payload = {
  metadata: {
    ...
    generatedAt,
    selectionMethod: "جمع عميق عبر واجهة Internet Archive البرمجية بالمؤشر؛ يقبل الديوان إذا صرّح العنوان به أو ربطت البيانات الوصفية عنوان السجل نفسه بديوان مستقل، مع استبعاد الدراسات والسجلات الإدارية والمجاميع أو الأجزاء غير المستقلة والمواد غير العربية.",
    collectionScope: "أربعة مسارات بحث في حقول العنوان والموضوع والوصف بلا اقتصار على وسم اللغة، مع حد تشغيل مسؤول لكل مسار وإزالة التكرار داخلياً قبل الدمج.",
    paging: { api: endpoint, batchSize, maxRecordsPerQuery, plannedQueries: requestPlan.length },
    requests,
  },
  materials,
};
const audit = {
  ...
  selectedSample: materials.slice(0, 80),
