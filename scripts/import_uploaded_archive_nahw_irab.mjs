/*
 * استيراد محافظ لدفعة النحو والإعراب من Internet Archive.
 * لا يضيف مادة إلا إذا ثبتت عربيتها من حقل اللغة أو بيانات وصفية عربية واضحة،
 * وتحققت صلتها المباشرة بالنحو أو الإعراب، مع منع التكرار بالمعرّف والرابط والعنوان المطبّع.
...
const INPUT_PATH = process.env.INPUT_PATH || "/home/ubuntu/upload/pasted_file_uhyDCG_archive_nahw_irab_book_titles.json";
...
const AUDIT_PATH = process.env.AUDIT_PATH || "/home/ubuntu/archive_nahw_irab_import_audit.json";
const CACHE_PATH = process.env.CACHE_PATH || "/home/ubuntu/archive_nahw_irab_metadata_cache.json";
...
const EDUCATIONAL_WORKSHEET_PATTERN = /مراجعة نهائية|المراجعة النهائية|اختبار(?:ات)?|امتحان(?:ات)?|إجابة (?:نموذجية|الامتحان)|الثانوية العامة|الصف\b|بكالوريا|(?:لتلاميذ|تلاميذ|المدارس|المرحلة|الإعدادية|الابتدائية|الاعدادية|الابتدائية)|ملخص رسالة|تدريبات (?:محلولة|على)/i;
...
if (!Array.isArray(inputBooks)) throw new Error("ملف دفعة النحو والإعراب لا يحتوي مصفوفة books صالحة.");
...
for (const { book, metadata } of snapshot) {
  ...
  if (tags.length === 0 || !NAHW_IRAB_PATTERN.test([title, subjectText, descriptionText].join(" ")) || EXCLUDED_TOPIC_PATTERN.test(catalogText)) {
    rejected.push(details(book, "لا يثبت العنوان والبيانات صلة المادة المباشرة بالنحو أو الإعراب", metadata));
    continue;
  }
  ...
  const candidate = {
    ...
    relativePath: `أرشيف/النحو والإعراب/${identifier}/${safeTitleForPath(title)}.pdf`,
    sourceUrl,
...
const output = {
  ...
  metadata: {
    ...corpus.metadata,
    selectionMethod: `${corpus.metadata.selectionMethod} أضيفت مواد عربية منتقاة من Internet Archive في النحو والإعراب، بعد التحقق من بيانات اللغة والموضوع ومنع التكرار متعدد المفاتيح.`.trim(),
    statistics: {
...
const audit = {
  ...
  policy: {
    ...
    topical: "تُقبل المواد المتصلة مباشرة بالنحو أو الإعراب فقط.",
