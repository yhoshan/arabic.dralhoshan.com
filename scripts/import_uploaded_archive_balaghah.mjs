/*
 * استيراد محافظ لدفعة البلاغة وعلوم العربية من Internet Archive.
 * لا يضيف مادة إلا إذا ثبتت عربيتها من حقل اللغة أو بيانات وصفية عربية واضحة،
 * وتحققت صلتها بعلوم العربية، مع منع التكرار بالمعرّف والرابط والعنوان المطبّع.
...
const INPUT_PATH = process.env.INPUT_PATH || "/home/ubuntu/upload/archive_balaghah_book_titles.json";
...
const AUDIT_PATH = process.env.AUDIT_PATH || "/home/ubuntu/archive_balaghah_import_audit.json";
const CACHE_PATH = process.env.CACHE_PATH || "/home/ubuntu/archive_balaghah_metadata_cache.json";
...
const LITERATURE_PATTERN = /أدب|ادب|شعر|عروض|قافية|نقد أدبي|النقد الأدبي|ديوان/i;
...
const EDUCATIONAL_WORKSHEET_PATTERN = /مراجعة نهائية|المراجعة النهائية|اختبار(?:ات)?|امتحان(?:ات)?|إجابة (?:نموذجية|الامتحان)|الثانوية العامة|الصف (?:الأول|الثاني|الثالث)|بكالوريا|تدريبات (?:محلولة|على)/i;
...
async function hydrateMetadata(books, cache) {
  ...
  for (const batch of batches) {
    const fetched = await fetchMetadataBatch(batch);
    for (const identifier of batch) {
      cache[identifier] = fetched[identifier] || { _fetchError: "لم يعثر بحث أرشيف على بيانات وصفية للسجل" };
    }
...
if (!Array.isArray(inputBooks)) throw new Error("ملف دفعة البلاغة لا يحتوي مصفوفة books صالحة.");
...
for (const { book, metadata } of snapshot) {
  ...
  if (!title || !identifier || !/^https:\/\/archive\.org\/details\//i.test(sourceUrl)) {
  ...
  const classificationText = [title, descriptionText].join(" ");
  ...
  const catalogText = [title, subjectText, descriptionText, creatorText].join(" ");
  ...
  if (EDUCATIONAL_WORKSHEET_PATTERN.test([title, subjectText].join(" "))) {
  ...
  if (tags.length === 0 || !ARABIC_TOPIC_PATTERN.test([classificationText, subjectText].join(" ")) || (EXCLUDED_TOPIC_PATTERN.test(catalogText) && !RHETORIC_PATTERN.test(classificationText))) {
    rejected.push(details(book, "لا يثبت العنوان والبيانات صلة المادة بالبلاغة أو علوم العربية", metadata));
    continue;
  }
  ...
  const candidate = {
    ...
    relativePath: `أرشيف/البلاغة وعلوم العربية/${identifier}/${safeTitleForPath(title)}.pdf`,
    sourceUrl,
  ...
  const candidateTitleAuthor = titleAuthorKey(candidate);
...
const acceptedByTag = Object.fromEntries(
  ["بلاغة", "نحو", "صرف", "معجم لغوي", "دراسات لغوية", "شعر وأدب"].map((tag) => [
    tag,
...
const output = {
  ...
  metadata: {
    ...corpus.metadata,
    selectionMethod: `${corpus.metadata.selectionMethod} أضيفت مواد عربية منتقاة من Internet Archive في البلاغة وعلوم العربية، بعد التحقق من بيانات اللغة والموضوع ومنع التكرار متعدد المفاتيح.`.trim(),
    statistics: {
...
const audit = {
  ...
  policy: {
    ...
    topical: "تُقبل البلاغة وعلوم العربية ذات الصلة فقط.",
